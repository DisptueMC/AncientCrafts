package cn.disputemc.utils;

import cn.disputemc.ancientcrafts;
...
public class RecipeManagerTools {
    private static final Gson PARSER = new Gson();

    public static void registerRecipeToServer(ancientcrafts mainClass, String recipe_path) {
        URL url = mainClass.getClass().getClassLoader().getResource(recipe_path);
        if (url == null) {
            ancientcrafts.log.error("插件jar资源文件错误，请重新下载!");
            return;
        }
    ...
    private static Recipe parseAddonRecipe(Map<?, ?> recipeMap) {
        if (recipeMap.get("format_version").equals("1.12") && recipeMap.containsKey("minecraft:recipe_shaped")) {
            ...
            ancientcrafts.log.error("配方文件必须是1.12版本的格式!");
            return null;
        }
    }
}
